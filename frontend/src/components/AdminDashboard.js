          ...
          <TabsContent value="notifications" className="space-y-6">
            <Card className="card-hover">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Monitor all notification attempts and delivery status
                    </CardDescription>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Button
                      ...
                      disabled={refreshing}
                      id="admin-notifications-refresh-button"
                    >
                      <RefreshCw className={`h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} />
                      <span className="ml-2">Refresh</span>
                    </Button>
              ...
              <CardContent>
                ...
                {/* Search and Filter Controls */}
                <div className="flex flex-col sm:flex-row gap-4 mb-6" id="admin-notifications-filters">
                  <div className="flex-1" id="admin-notifications-search-container">
                    <div className="relative">
                      <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
                      <Input
                        placeholder="Search by user ID, message, or sender..."
                        value={notificationSearchTerm}
                        onChange={(e) => setNotificationSearchTerm(e.target.value)}
                        className="pl-10"
                        id="admin-notifications-search-input"
                      />
                    </div>
                  </div>
                  <div className="flex gap-2" id="admin-notifications-filter-controls">
                    <Select value={notificationTypeFilter} onValueChange={setNotificationTypeFilter}>
                      <SelectTrigger className="w-40" id="admin-notifications-type-filter">
                        <Filter className="h-4 w-4 mr-2" />
                        <SelectValue placeholder="Type" />
                      </SelectTrigger>
                      <SelectContent>
                        ...
                        <SelectItem value="ride_accepted">Ride Accepted</SelectItem>
                        <SelectItem value="ride_started">Ride Started</SelectItem>
                        <SelectItem value="driver_arrived">Driver Arrived</SelectItem>
                        <SelectItem value="balance_transaction">Balance Transaction</SelectItem>
                      </SelectContent>
                    </Select>
                    <Select value={notificationStatusFilter} onValueChange={setNotificationStatusFilter}>
                      <SelectTrigger className="w-32" id="admin-notifications-status-filter">
                        <SelectValue placeholder="Status" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">All Status</SelectItem>
                    ...
                    <Button
                      ...
                      id="admin-notifications-clear-filters"
                    >
                      Clear
                    </Button>
                  </div>
                    ...
                    <TableBody>
                      {getFilteredNotifications().length === 0 ? (
                        ...
                        getFilteredNotifications().map((notification) => (
                          <TableRow key={notification.id} id={`admin-notification-row-${notification.id}`}>
                            <TableCell id={`admin-notification-timestamp-${notification.id}`}>
                              {new Date(notification.created_at).toLocaleString()}
                            </TableCell>
                            <TableCell id={`admin-notification-type-${notification.id}`}>
                              <Badge variant="outline">
                                {notification.type.replace('_', ' ')}
                              </Badge>
                            </TableCell>
                            <TableCell id={`admin-notification-recipient-${notification.id}`}>
                              {notification.user_id}
                            </TableCell>
                            <TableCell id={`admin-notification-sender-${notification.id}`}>
                              {notification.sender_name || 'System'}
                            </TableCell>
                            <TableCell id={`admin-notification-message-${notification.id}`} className="max-w-xs truncate">
                              {notification.message}
                            </TableCell>
                            <TableCell id={`admin-notification-status-${notification.id}`}>
                              <Badge 
                                variant={notification.delivered ? "default" : "secondary"}
                                className={notification.delivered ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}
                              >
                                {notification.delivered ? 'Delivered' : 'Pending'}
                              </Badge>
                            </TableCell>
                            <TableCell id={`admin-notification-actions-${notification.id}`}>
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => viewNotificationDetails(notification)}
                                id={`admin-notification-view-${notification.id}`}
                              >
